...
export default function AdminDossierCategories() {
  ...
  const createMutation = useMutation({
    mutationFn: (name) => appClient.functions.invoke('saveDossierCategory', { name }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['adminDossiers'] }); setNewName(''); },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, name }) => appClient.functions.invoke('saveDossierCategory', { id, name }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['adminDossiers'] }); setEditingId(null); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => appClient.functions.invoke('deleteDossierCategory', { id }),
  ...
  const handleDragEnd = async (result) => {
    ...
    await Promise.all(reordered.map((cat, idx) =>
      appClient.functions.invoke('saveDossierCategory', { id: cat.id, name: cat.name, order: idx })
    ));
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-white text-sm font-medium tracking-wide">Dossier Categories</h3>
      ...
      {/* List */}
      {ordered.length === 0 && (
        <p className="text-white text-sm text-center py-6">No categories yet.</p>
      )}
                    ...
                    >
                      ...
                      {editingId === cat.id ? (
                        ...
                        <>
                          ...
                          <button onClick={() => startEdit(cat)} className="text-white hover:text-white transition-colors">
                            <Pencil size={13} />
                          </button>
                          <button onClick={() => deleteMutation.mutate(cat.id)} className="text-white hover:text-red-400 transition-colors">
                            <Trash2 size={14} />
                          </button>
                        </>
                      )}
